...
function createBoard() {
  ...
  document.getElementById('matches').innerHTML =  0 + " / " + cards.length/2; //displays the matches count at the bottom
    ...
    for (var i = cards.length -1; i >= 0; i--) {
        ...
        card.addEventListener('click', flipOverCard); //listen for when the player selects a card
...
function flipOverCard(e) {
  if (cardsInPlay.length === 2) { //checks for unmatched cards
    turnFaceDown(); //turns them face down
    return;
  }
...
function turnFaceDown() {
    for (i = 0; i < cardsInPlay.length; i++) {
      var flipCards = document.getElementById(cardsInPlay[i]);
			...
			//flipCards.classList.remove('flipUp'); //remove class flipUp
...
function isMatch() {
  ...
  if (currentCard === lastCardPlayed) { //if they match, alert the player:
    ...
    for (i = 0; i < cardsInPlay.length; i++) {
      var card = document.getElementById(cardsInPlay[i]);
      card.removeEventListener('click', flipOverCard); //remove the flipOver event listener from the two matched cards
    }
    ...
    document.getElementById('matches').innerHTML = matches + " / " + cards.length/2; //add new match count to matches html id
    	...
    	document.getElementById('success').innerHTML = "Try again."; //if cards don't match, alert player
      cardTypes = []; //empty cardTypes
  }
}
